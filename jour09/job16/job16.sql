-- CONSIGNE JOB 16 permet dde récupérer le nom de l'étage ayant la salle avec la plus grande capacité (et afficher aussi le nom de cette salle
-- ainsi que sa capacité). Dans ce résultat, la colonne “nom” de la salle doit être renommée en “Biggest Room”

-- Requête :
-- sélectionne le nom de la salle (renommé), le nom de l’étage et la capacité
SELECT 
-- AS renomme la colonne du nom de la salle
  salles.nom AS 'Biggest Room',
-- affiche le nom de l’étage correspondant
  etage.nom AS nom_etage,
-- affiche la capacité de la salle
  salles.capacite
-- relie les tables "salles" et "etage"
FROM salles
INNER JOIN etage ON salles.id_etage = etage.id
-- garde seulement la salle ayant la capacité maximale
...
--
-- Hôte : 127.0.0.1:3307
-- Généré le : ven. 03 oct. 2025 à 09:38
